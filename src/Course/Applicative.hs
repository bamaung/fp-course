...
filtering ::
  ...
  -> List a
  -> k (List a)
filtering mp = foldRight f (pure Nil)
  where f a = lift2 (\p -> if p then (a:.) else id) (mp a)
  -- where f a mb = (\p -> if p then (a:.) else id) <$> (mp a) <*> mb
